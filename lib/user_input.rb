...
puts "What would you like to be the size of your grid? (such as height, width => ex: 5,5)"
  ...
  navigator.select_grid(grid_dimensions[0].to_i, grid_dimensions[1].to_i)
  navigataor.grid.grid.each_index do |i|
...
puts "Please enter your directios for the rover (such as you can only use the ltters L for left, R for right and M for move forward"
  directions = get.chomp
  ...
  puts "-----"
  navigator.grid.grid.each_index do |e|
    p navigator.grid.grid[e]
  end
  puts "-----"
